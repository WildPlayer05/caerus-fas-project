<div class="w-full sm:w-11/12 max-w-[355px] bg-white border border-gray-200 rounded-lg shadow contract-card" type="{{$contract->type}}" energyType="{{$contract->type}}">
    ...
    <div class="p-5">
        <a>
            <h5 class="mb-2 text-center text-2xl font-bold tracking-tight text-gray-900">
                @if($contract->type=='home') Private @else Business  @endif
            </h5>
        </a>
        <ul role="list" class="mb-8 space-y-4 text-left">
            ...
            <div class="grid grid-cols-2 gap-y-4 text-sm/[15px]">
            <p class="font-light text-gray-500 col-span-2">
                Gas Price: 
            ...
            <p class="font-light text-gray-500 justify-self-center">
                KWh: 
                @if($contract->KWh == null)
                    -
                @else
                    {{$contract->KWh}} KW/h
                @endif
            ...
            <p class="font-light text-gray-500 col-span-2">
                Minimum Duration: 
                @if($contract->minimumDuration == null)
                    -
                @else
                    {{$contract->minimumDuration}} days
                @endif
            </p>
            </div>
        </ul>
        ...
        <div class="justify-center flex">
            <a href="{{route('user.stipulate', $contract->id)}}" class="text-blue-600 hover:underline font-medium text-lg inline-flex items-center">Stipulate
                contract
